...
For closed-loop dynamic navigation, compare incremental D* Lite replanning
with a fresh A* plan while continuing Stanley or MPC control:

```bash
./build/apps/dynamic_navigation \
    --map autoplanner/data/maps/simple_50x50.txt \
    --controller mpc --frames 5 --steps-per-frame 40 \
...
To benchmark local control around the same global planner, compare the
nominal controller, DWA, and MPPI on static and moving-obstacle scenarios:
...
python scripts/plot_benchmark.py \
    results/benchmark/planning_results.csv \
    --output_dir results/benchmark/images
```
