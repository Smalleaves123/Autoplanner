...
// Prevent corner-cutting: a diagonal move is only valid if both
// adjacent cardinal cells are free.
...
WeightedAStarPlanner::WeightedAStarPlanner(double weight, bool allow_diagonal)
    : weight_(weight)
    , allow_diagonal_(allow_diagonal)
    , heuristic_(std::make_unique<EuclideanHeuristic>()) {}

std::string WeightedAStarPlanner::name() const {
    return "weighted_astar";
}

void WeightedAStarPlanner::setHeuristic(std::unique_ptr<Heuristic> heuristic) {
    heuristic_ = std::move(heuristic);
}

void WeightedAStarPlanner::setWeight(double weight) {
    weight_ = weight;
}

PlannerResult WeightedAStarPlanner::plan(
    const GridMap& map,
    const Point2i& start,
    const Point2i& goal
) {
    ...
    // f = g + w * h — the weight amplifies the heuristic, pulling
    // the search more aggressively toward the goal.
    const double h_start = heuristic_->compute(start, goal);
    open.push({start_index, weight_ * h_start});
    ...
    while (!open.empty()) {
        ...
        for (std::size_t i = 0; i < dxs.size(); ++i) {
            ...
            if (tentative_g < g_score[static_cast<std::size_t>(neighbor_index)]) {
                ...
                // Key difference from A*: f = g + w * h.
                const double f = tentative_g + weight_ * h;
