...
// Lightweight node for the priority queue — ordered by cost g.
struct QueueNode {
    int index = -1;
    double g = 0.0;
...
// Diagonal moves are only allowed if both adjacent cardinal cells are free,
// preventing corner-cutting through walls.
...
DijkstraPlanner::DijkstraPlanner(bool allow_diagonal)
...
PlannerResult DijkstraPlanner::plan(
    const GridMap& map,
    const Point2i& start,
    const Point2i& goal
) {
    ...
    while (!open.empty()) {
        ...
        // Skip if already expanded (closed-set check).
        ...
        // Goal reached — path reconstruction will happen after the loop.
        ...
        for (std::size_t i = 0; i < dxs.size(); ++i) {
            ...
            if (tentative_g < g_score[static_cast<std::size_t>(neighbor_index)]) {
                ...
                // No heuristic — priority is pure g.
                open.push({neighbor_index, tentative_g});
            }
        }
    }
